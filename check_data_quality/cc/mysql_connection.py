...
if __name__ == '__main__':
    ...
    sql_query = 'select upload_file_s3_path ' \
                ...
                'WHERE object_name = %s and prepare_start_datetime >= %s ' \
                'and status != %s and file_name is null ' \
                'order by prepare_start_datetime asc'

    temp_list = web_dev2.select_advanced(sql_query,
                                         ('object_name', "common_order"),
                                         ('prepare_start_datetime',
                                          "2019-05-03 00:00:00"),
                                         ('status', '4'))
    ...
    with open("test.txt", "w+") as file:
        ...
        for i in range(len(temp_list) - 1):
            # new_line = temp_list[i] + "\n"
            new_line = 'hdfs dfs -ls -h ' + temp_list[i] + '* ' + temp_list[-1] + "\n"
            file.write(new_line)
        print("Done")
