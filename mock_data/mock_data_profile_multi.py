...
def mock_user(seq, dt, row_cnt):
    ...
    with open('test_profile_new_%s.csv' % dt, 'w') as f:
        for num in range(1, row_cnt + 1):
            ...
            origin_birth_date = faker.date_of_birth(tzinfo=None, minimum_age=0,
                                                    maximum_age=100)
            birth_date = str(origin_birth_date)
            birth_date_year = origin_birth_date.strftime("%Y")
            birth_date_year_month = origin_birth_date.strftime("%Y-%m")
            birth_date_month_day = origin_birth_date.strftime("%m-%d")
            ssn = faker.ssn()
            job = faker.job()
            salary = round(random.randint(90000, 120000) / 1000) * 100
            ss_fund = random.random() * 1000
            f.write(
                '%s,%s,%s,%s,%s,%s,%s,%s,%s, %s,%s, %s, %s, %s, %s, %s\n' % (
                    seq, user_id, user_name, address, gender, email, birth_date,
                    birth_date_year, birth_date_year_month,
                    birth_date_month_day,
                    phone,
                    ssn, job, salary, ss_fund, dt))
            seq += 1
            if num % 10000 == 0:
                f.flush()
...
if __name__ == "__main__":
    start_dt = datetime.datetime(2020, 3, 3)
